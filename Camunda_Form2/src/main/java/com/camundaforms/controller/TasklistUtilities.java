...
//	}
	...
	public List getALLActiveListOfTasksCustomize() throws Exception {
			...
			hitsObjList = (List) hitsObj.get("hits");
			Map sourceObj = null;
			Set<String> allInsIdList = new HashSet<String>();
			List<String> proDefinlist = new ArrayList<String>();
			for (Map ref : hitsObjList) {
				sourceObj = (Map) ref.get("_source");
				Map valueObj = (Map) sourceObj.get("value");
				long processDefinitionKey = (long) valueObj.get("processDefinitionKey");
				String nwprocessDefinitionKey = String.valueOf(processDefinitionKey);
				proDefinlist.add(nwprocessDefinitionKey);
			}
			for (String refkey : proDefinlist) {
				for (Map refe : hitsObjList) {
					sourceObj = (Map) refe.get("_source");
					Map valueObj = (Map) sourceObj.get("value");
					long processDefinitionKey = (long) valueObj.get("processDefinitionKey");
					String nwprocessDefinitionKey = String.valueOf(processDefinitionKey);
					if (nwprocessDefinitionKey.equals(refkey)) {
						long processIns = (long) valueObj.get("processInstanceKey");
						String ins = String.valueOf(processIns);
						allInsIdList.add(ins);
					}
				}
			}
			Map customHeaders = null;
			Set<Long> idsWithOnlyCreated = new HashSet<Long>();
			for (String ref1 : allInsIdList) {
				for (Map refs : hitsObjList) {
					sourceObj = (Map) refs.get("_source");
					Map valueObj = (Map) sourceObj.get("value");
					long processInstanceKey = (long) valueObj.get("processInstanceKey");
					String nwprocessInstanceKey = String.valueOf(processInstanceKey);
					if (nwprocessInstanceKey.equals(ref1)) {
						Map<Long, Set<String>> stateMap = new HashMap<>();
						for (Map activeTaskObj : hitsObjList) {
							Map _source = (Map) activeTaskObj.get("_source");
							long ky = (long) _source.get("key"); // important
